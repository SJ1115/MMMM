...
def binomial(n, p):
    assert n > 0, "number of sampling 'n' should be positive."
    assert 1 >= p >= 0, "probability 'p' should be in [0, 1]."
    
    n = int(n)
    
    if p and n:
        out =  sum(random() < p for i in range(n))
        while not out: ## not to return 0
            out =  sum(random() < p for i in range(n))
        return out
    else:
        return 0

def resample_from_binomial(l:int):
    if l:
        n, p = l**(1.5)/(l**.5 - 1 +1e-7), 1 - l**-.5 +1e-7
        l = np.random.binomial(n, p)
        return l#max(l, 1)
    return 0

def resample_from_poisson(l:int):
    l =  np.random.poisson(l)
    return l#max(l, 1)

def resample_from_geometric(l:int):
    if not l:# l == 0
        return 0
...
class DataCollatorForMMMM:
    def __init__(self, 
            ...
            max_length:int=512,
            mask_with_gaussian:bool=False,
            resample_length:str=None,
        ...
        label_names = set([label[2:] for label in id2label.values() if len(label)>2])
        ...
        self.label2head = { # label id(on entity) : header token id(on tokenizer)
            label2id[f"B-{label}"] : tokenizer.convert_tokens_to_ids(head)
        ...
        self._resample_flag = False
        if resample_length:
            resample_length = resample_length.lower()
            assert resample_length in ['none', 'binomial', 'poisson', 'geometric'], f"masking strategy should be one of 'none', 'binomial', 'poisson' or 'geometric', \'{resample_length}\' was found."
            
            match resample_length:
                case 'none':
                    self.get_mask_len = lambda x:x
                case 'binomial':
                    self.get_mask_len = resample_from_binomial
                    self._resample_flag = True
                case 'poisson':
                    self.get_mask_len = resample_from_poisson
                    self._resample_flag = True
                case 'geometric':
                    self.get_mask_len = resample_from_geometric
                    self._resample_flag = True
    ...
    def __call__(self, items):
        ...
        for item in items:
            ...
            while i<len(item['input_ids']):
                ## Additional mask: only when 'checkers' exists
                if 'checkers' in item:
                ...
                if (item['labels'][i] > 0) and \
                    ...
                    if self._resample_flag:
                        l = 0
                        while(j<len(item['labels'])) and (item['labels'][j] == entity_label+1):
                            j += 1
                            l += 1

                        l = self.get_mask_len(l)
                        
                        tokens += [self.MASK] * l
                        labels += [ item['input_ids'][i]+1 ] * l
    ...
    def generate(self, result_dir:str=None, rounds:int=3):
        ...
        for _ in range(rounds):
            for data in self.loader:
                ...
                for t_in, t_out in zip(data['input_ids'].to('cpu'), out.logits.to('cpu')):
                    ...
                    while i < len(t_in):
                        ...
                        if tok not in self.label2head.values():
                            ...
                            o_labels_out.append(ori_labels[k])
                            added.append(0)
                        else:
                            j = i+1
                            l = 0 # len of generated tokens
                            ...
                            ## Append connected labels
                            o_labels_out += [ ori_labels[k] ] + [ ori_labels[k]+1 ]*(l-1)
                            next_k = k

                            while (next_k+1 < len(ori_labels)) and \
                                  (ori_labels[next_k+1] == ori_labels[k]+1):
                                next_k += 1
                            k = next_k
                ...
                assert len(result) == len(o_labels_out) , f"label len should be equal to input len={len(result)}, but is {len(o_labels_out)}"#== len(added) 
                augmented.append({
                    'input_ids':result,
                    'labels':o_labels_out,
                    'added_tokens':added,
...
def filter_MMMM_augmentation(model, tokenizer, augmented, id2label, out_dir=None):
    ...
    for item in tqdm(augmented, desc="filtering..."):
        ...
        flag = True
        for sign, pred, answ in zip(is_generated, logits, labels):
            if sign and id2label[answ].startswith("B-"):
                if pred != answ:
                    flag = False
                    break
